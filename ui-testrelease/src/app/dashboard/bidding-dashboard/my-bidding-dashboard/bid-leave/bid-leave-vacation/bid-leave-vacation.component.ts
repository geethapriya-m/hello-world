...
import { BidLeaveService } from 'src/app/services/manage-bid-schedule/create-new-bid-schedule/bid-leave/bid-leave.service';
...
export class BidLeaveVacationComponent implements OnInit {
  ...
  this.headerTitleService.setBackUrl(straightlines_io_apis.apis.select_bid_leave_option);
      ...
      this.bidLeaveSer.getBidVacationDataBasedOnBidScheduleId(Number(this.bid_schedule_id)).subscribe(async (res)=>{
        ...
        if(this.empId==temp[v].empidref){
          ...
          if(this.totalgetVacationHours>=this.totalEmpVacationHours || this.remainingVacationHours<8){
            ...
            console.log(this.remainingVacationHours)
...
getAllData(){
  ...
  getCancelLine(date,cRDO){
    if(cRDO=='nrdo'){
      ...
      if(date.getDate()<10){
        return 'strikethrough-single-rdo'
      }else{
        return 'strikethrough-rdo'
      }
    }
  ...
  tempArrn=res
    ...
    if(tempObjn.vacationbidstatus=='Eligible'){
      ...
      for(var i=0;i<lastDate.length;i++){
        ...
        if(tempObj.vcationhours>8){
          confirmationMsg.push((startDate.getMonth()+ + +1)+'/'+startDate.getDate()+'-'+(endDate.getMonth()+ + +1)+'/'+endDate.getDate())
        }else{
          if(tempObj.vcationhours>0){
            confirmationMsg.push((startDate.getMonth()+ + +1)+'/'+startDate.getDate())
          }
        }
      ...
      for(var v=0;v<tempArrTwo.length;v++){
        temp=tempArrTwo[v]
        if(temp.vacationstartdate!=null && temp.vacationenddate!=null && temp.vacationstartdate!=undefined && temp.vacationenddate!=undefined){
          countVacation++
          var tempNewObj={
