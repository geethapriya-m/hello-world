...
export class BidScheduleActionSheetComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(bidSchedule.bidschstartdate!=undefined && bidSchedule.bidschstartdate!=null){
      if(bidSchedule.roundmap.length>0){
        if(bidSchedule.roundmap[0].roundstarttime!=undefined && bidSchedule.roundmap[0].roundstarttime!=null){
          start=new Date(Number(bidSchedule.bidschstartdate.split('-')[0]),Number(bidSchedule.bidschstartdate.split('-')[1])+ - +1,Number(bidSchedule.bidschstartdate.split('-')[2])+ - +1,Number(bidSchedule.roundmap[0].roundstarttime.split(':')[0]),Number(bidSchedule.roundmap[0].roundstarttime.split(':')[1]),0)
        }else{
          start=new Date(Number(bidSchedule.bidschstartdate.split('-')[0]),Number(bidSchedule.bidschstartdate.split('-')[1])+ - +1,Number(bidSchedule.bidschstartdate.split('-')[2])+ - +1,0,0,0)
        }
      }else{
        start=new Date(Number(bidSchedule.bidschstartdate.split('-')[0]),Number(bidSchedule.bidschstartdate.split('-')[1])+ - +1,Number(bidSchedule.bidschstartdate.split('-')[2])+ - +1,0,0,0)
      }
    ...
    if(new Date().getTime()<start.getTime()){
      this.checkBidShceduleInProgress=true
    }else{
      this.checkBidShceduleInProgress=false
    }
  }
  next() {
    const r = [];
  ...
  *[Symbol.iterator]() {
  ...
  const confirm = await this.alertCtrl.create({
    ...
    localStorage.setItem('editBidSchedule',JSON.stringify(bid_schedule_data))
