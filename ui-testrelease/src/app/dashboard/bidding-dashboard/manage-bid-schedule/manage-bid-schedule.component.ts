  ...
  get year(){
    ...
    allBidScheduleList(){
      this.newBidScheduleSer.getAllBidScheduleData(this.user_data.id).subscribe((res)=>{
        ...
        if(this.all_Bid_schedule_list.length>0){
          this.all_Bid_schedule_list.sort((a, b)=>{return b.bidschid - a.bidschid});
        }
    ...
    async editBidScheduleFromList(bs){
...
return await modal.present();
    ...
    multiDimensionalUniqueForBidSchedule(data,bidSchedule) {
      ...
      localStorage.setItem('setUpBidScheduleOne',JSON.stringify(temp))
      this.navCtrl.navigateForward([straightlines_io_apis.apis.setUp_bid_parameters])
    }
